import 'package:flutter/material.dart';

class MainColors {
  Color white = Colors.white;
  Color appGreen = const Color.fromRGBO(0, 220, 167, 1);
  Color lightBlue = const Color.fromRGBO(220, 237, 255, 1);
  Color grey = Colors.grey.shade500;
}

class SplashColors {
  Color loaderColor = const Color.fromRGBO(0, 82, 168, 1);
  Color bgColor = const Color.fromRGBO(248, 248, 248, 1);
}

class OnBoardColors {
  static Color bgColor = const Color.fromRGBO(0, 82, 168, 1);
  static Color virtualColor = const Color.fromRGBO(0, 220, 167, 1);
}

class HomeColors {
  Color nameColor = const Color.fromRGBO(6, 31, 58, 1);
}
